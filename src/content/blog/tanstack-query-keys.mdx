...
```typescript
["todos", "list", { filters: "all" }][("todos", "list", { filters: "done" })][ // 전체 목록 // 완료된 목록
  ("todos", "detail", 1)
][("todos", "detail", 2)]; // 1번 할일 상세 // 2번 할일 상세
```
...
**오타?** `todoKeys.detial`이면 TypeScript가 바로 에러를 냅니다. **구조 변경?** `todoKeys` 객체 하나만 수정하면 끝입니다. **기억 의존?** IDE 자동완성이 모든 옵션을 보여줍니다.
...
직접 팩토리 객체를 만드는 것이 번거롭다면, 커뮤니티 라이브러리인 [@lukemorales/query-key-factory](https://github.com/lukemorales/query-key-factory)를 사용할 수도 있습니다.
...
소규모 프로젝트라면 1단계로도 충분합니다. 쿼리가 10개를 넘어가기 시작하면 2단계를, TypeScript를 적극 활용한다면 3단계를 고려해보세요.
