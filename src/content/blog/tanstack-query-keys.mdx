...
이 문제를 해결하는 **Query Key Factory 패턴**을 단계별로 알아봅니다. 이 글을 읽으면 오타 걱정 없이 타입 안전한 Query Key를 설계하고, `queryOptions()`로 키와 함수를 하나로 묶어 유지보수성을 높일 수 있습니다. TanStack Query 메인테이너인 TkDodo(Dominik Dorfmeister)가 제안한 이 패턴은 커뮤니티에서 사실상 표준으로 자리잡았습니다.
...
이렇게 하면 `invalidateQueries`의 **접두사 매칭(prefix matching)**을 활용할 수 있습니다.
